...
from util import check_listening_on_port
...
class P4RuntimeSwitch(P4Switch):
    ...
    def start(self, controllers):
        ...
        #args.extend(["--restore-state", "s1-commands.txt"])
        ...
        P4Switch.device_id += 1
        args.extend(["--thrift-port", str(P4Switch.device_id + 9000)])
        if self.json_path:
        ...
        cmd = " ".join(args)
        cmd += " --load-modules ../build/externs/libp4ipsec.so," \
            + "../build/externs/libp4macsec.so" # TODO make parameter
        info(cmd + "\n")
